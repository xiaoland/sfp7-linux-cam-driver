...
%define with_debuginfo_manual 0
%define with_debuginfo_rpm 0
%define ARCH x86_64
%define KERNELRELEASE 6.19.8-sfp7cam.p1.fc42.x86_64
...
%post
set -eu
saved_entry=$(/usr/bin/grub2-editenv - list | sed -n 's/^saved_entry=//p')
if [ -z "$saved_entry" ]; then
	echo 'Refusing kernel install without a saved GRUB default' >&2
	exit 1
fi
restore_saved_entry() {
	/usr/bin/grub2-editenv - set "saved_entry=$saved_entry"
}
trap restore_saved_entry EXIT
...
%changelog
* Wed Sep 23 2026 xiaoland <37663413+xiaoland@users.noreply.github.com>
